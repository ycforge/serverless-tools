...
скоррелировать с ответом (ошибки возвращают `trace_id` в теле). `count` в
gateway-спеке объявлен строкой намеренно: YC API Gateway материализует
integer-default как число, а коннектор fail-fast требует строковых значений
параметров (иначе 502).
