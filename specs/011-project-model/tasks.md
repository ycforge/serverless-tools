...
- [ ] T003 [P] Update `packages/pilot/test/unit/zero-dependency.test.ts` — keep the `src/contracts` relative-import check; scope the package.json dependency assertion to the `@ycforge/pilot/contracts` subpath so it still enforces that the contracts module has no runtime deps without failing now that the pilot root package gains the `yaml` dependency (add a comment noting the split)
