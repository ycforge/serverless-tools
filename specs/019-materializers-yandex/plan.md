...
- Materializers do NOT validate file existence (trust builder output / spec 018; archive missing → error surfaces at `terraform apply`, not materialize — spec Edge Cases). Exception: invalid artifact VALUE shape (missing required field) → `YMT_INVALID_ARTIFACT_VALUE`; invalid `queueUrl` → `YMT_INVALID_QUEUE_URL` (fail-fast, Constitution V).
...
- **filename sanitization**: `[a-zA-Z0-9_]` with underscore dedup/trim; original preserved in `key` (research D-RE-8).
...
- Whether `YMT_EMPTY_DIRECTORY` should surface as a warning through the diagnostics channel or only as a benign log (it does not fail materialization per FR-024).
