...
- [x] T050 Typecheck clean — `pnpm --filter @ycforge/builders-core typecheck` (`tsc --noEmit`, strict, noUncheckedIndexedAccess, exactOptionalPropertyTypes) → 0 ошибок: новые `DockerBuildConfig.image.no_push?`, `ParsedDockerConfig.noPush`, `BuildAndPushOptions.noPush?`, тест-d-блоки компилируются. **Depends**: T025, T044.
- [x] T051 Scoped lint clean — `pnpm exec eslint packages/builders-core/src packages/builders-core/test` → 0 NEW errors (паттерн 023 T144/026 T062: legacy-baseline корня за границами пакета не трогается). **Depends**: T050.
- [x] T052 Build — `pnpm --filter @ycforge/builders-core build` (tsup) → `dist/index.{js,cjs,d.ts}` + `dist/docker/index.{js,cjs,d.ts}` present; subpath `./docker` (exports :21-25 package.json) резолвится; новых external-зависимостей нет. **Depends**: T051.
...
- [x] T060 Convergence verdict — read-only аудит `/speckit.converge` выполнен: полный `pnpm exec vitest run` в packages/builders-core — **10 файлов / 89 passed + 1 skipped (90)** ALL GREEN (net delta: baseline 19 → 90; docker.spec.ts 30 it из них 14 прежних без правок, test-d 9 it; gated smoke 1 skipped); `tsc --noEmit` — 0 ошибок (exit 0); `pnpm exec tsup` — build success, dist-подпути `dist/docker/index.{js,cjs,d.ts}` присутствуют; scoped eslint `packages/builders-core/src packages/builders-core/test` — 0 **NEW** errors (прежние ошибки в 027-touched файлах `fake-bins.ts:145` и `test-d.ts:8` устранены в a11127d); прежний `docker.spec.ts` (14 it) зелёный без единой правки (SC-005, diff 359+/0−); test-d + audit-it подтверждают SC-007 (`DockerArtifactValue` frozen = `{ readonly image: string }`; JSON-audit: `no_push` boolean default false, const frozen); контракт аддитивен: только `image.properties.no_push` ({ type: 'boolean', default: false }) + description у `BLC_IMAGE_DIGEST_UNAVAILABLE` (const frozen), удалённых полей/типов 0; push-ветка `cli.ts:90-114` бит-в-бит, default без `no_push` → push неизменен; `git diff dev...HEAD` — только `packages/builders-core/**`, `specs/018-builders-core/contracts/builders-core.json`, `specs/027-docker-no-push/**`, `specs/README.md` (027 🚧); pilot/composer/materializers-core/nest-bridge/js-dev-tools — 0 диффов, `pnpm-lock.yaml` не тронут, `git status` чистый; gated smoke пропущен осознанно (probeDockerDaemon false — daemon на машине автора не запущен), 1 skipped; README-примечание A-2/NG-8 на месте (T044); hermetic-доказательства: no-push-пути без argv `push` (fake-docker журнал, SC-002), ENOENT/daemon-down → `BLC_BUILD_FAILED`, digest-absent → `BLC_IMAGE_DIGEST_UNAVAILABLE` (Artifact не возвращён); traceability: FR-001..FR-008 и US-1..US-5 → ≥1 задача [x], T011-T054 [x]; Constitution I (только builders-core), II (test-first; fake-docker characterization exception), III (аддитивность), V (fail-fast, строгий boolean, без warn-and-continue) соблюдены. **Verdict: Converged.** Legitimate deviations: (1) scoped eslint пакета не тотально «0 errors» — 2 pre-existing errors (`test/helpers/fixture-project.ts:86` unused `options`; `test/unit/zip-writer.spec.ts:8` unused `TempDir`) + 1 warning (`test/unit/nestjs-function.spec.ts:79` unused disable-директива) в файлах с 0 diff от dev (вне 027-границ); контракт T051 «0 NEW errors» соблюдён; (2) JSON-description у `BLC_IMAGE_DIGEST_UNAVAILABLE` расширен с накопленным «(FR-011).; also thrown…» — только description, const frozen (P-4/D-4); (3) `fake-bins.ts` fakeVite: снят лишний escape `\_unset\_` → `_unset_` (lint-fix a11127d; env-capture-поведение не изменилось); (4) gated daemon smoke (T043) пропущен — daemon выключен, документировано. **Depends**: T054.
