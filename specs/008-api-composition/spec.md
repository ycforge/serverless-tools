...
- **FR-012**: THE COMPOSER SHALL эмитить `components.securitySchemes` из каждой схемы `auth.yaml`, кроме типа `none`: `jwt` — `openIdConnect`-схема с производным `openIdConnectUrl: <issuer>/.well-known/openid-configuration` и вложенным jwt-authorizer (FR-013; вариант A, «Clarifications» 2026-09-05); `function` — HTTP-bearer placeholder (`{ type: http, scheme: bearer }`, семантика авторизации живёт в authorizer, FR-013); коллизия имени с существующим — fail-fast по FR-006. (US4/AC3–AC4; шов 007.)
...
| 7 | FR-012 «JWT/HTTP-bearer дескриптор» vs документация Yandex | Тип родительской `securityScheme` для схемы `jwt` при auth-эмиссии | **РЕШЕНО на этапе plan (2026-09-05, «Clarifications»)**: `openIdConnect` + производный `openIdConnectUrl: <issuer>/.well-known/openid-configuration` (вариант A); B (`http`/`bearer`, дословно FR-012) отклонён — недокументированная Yandex пара scheme/authorizer, риск отказа валидации при деплое; C (явное поле `openIdConnectUrl` в `auth.yaml`) отклонён — расширение контракта 007 ради одного поля |

## Clarifications

- **Сессия 2026-09-05** (фаза `/speckit-plan`):
  - **Q**: Тип родительской `securityScheme` для схемы `jwt` при auth-эмиссии: FR-012 дословно требовал «JWT/HTTP-bearer дескриптор», тогда как документация Yandex API Gateway требует `openIdConnect` для jwt-authorizer.
  - **A (вариант A, принят)**: запись `components.securitySchemes.<name>` для `jwt` = `{ type: openIdConnect, openIdConnectUrl: <issuer>/.well-known/openid-configuration, x-yc-apigateway-authorizer: { type: jwt, jwksUri, issuers, audiences, identitySource: { in: header, name: Authorization, prefix: "Bearer " } } }`. `openIdConnectUrl` выводится из `issuer` (`auth.yaml`, §12) по фиксированной конвенции `<issuer>/.well-known/openid-configuration`; рантайм-следствие — IdP обязан хостить OIDC discovery (иначе API Gateway не получит `jwks_uri`/ключи). FR-012 переформулирован; контрактная нота — в `contracts/api-composition.md`, детальный decision — `research.md` R5. Попадания на состояние контрактов/кодов ошибок нет (деталь эмиссии внутри стабильной границы authorizer-эмиссии; новые override-конфликты не вводит, новые коды `ComposeError` не требуются).

---

*Зависимости и разумные дефолты зафиксированы в Assumptions; требование-кандидат для /speckit-clarify не осталось — итоговые решения задокументированы выше (таблица «Точки неоднозначности» и секция «Clarifications»).*